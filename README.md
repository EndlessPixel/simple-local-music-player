...
搜索框右侧有两个模式按钮：**普通** 和 **正则**。点击 **正则** 即可切换到表达式模式，此时输入框会提示 `输入正则表达式，如 周杰伦|林俊杰`。

表达式就是 JavaScript 正则表达式，用来更灵活地筛选歌名。常用写法：
...
- 匹配的是**歌名（文件名去扩展名）**，不区分大小写。
- 输入非法正则时，输入框显示红框提示，且**不会过滤**列表，避免误清空。
- 想恢复简单搜索，点回 **普通** 即可。
