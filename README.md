...
搜索框右侧可切换 **普通** 与 **正则** 模式。**普通**模式支持多关键词（空格分隔、需全部命中），也能按子目录名搜索，并自动忽略空格 / `-` / `_` 等分隔符差异，例如 `周杰伦 晴天` 可以命中 `周杰伦 - 晴天.mp3`。**正则**模式输入框提示 `输入正则表达式，如 周杰伦|林俊杰`，即 JavaScript 正则，匹配**歌名（文件名去扩展名）**，不区分大小写：
...
<details>
<summary><b>浏览视图（文件夹 / 歌手 / 专辑）与重复检测</b></summary>
...
> 老库升级后服务会在首次启动时对全部歌曲做一次**元数据回填**（补专辑/音轨号/风格/年份，进度体现在启动日志的「回填」计数），之后照常增量扫描，不影响使用。
